...
# 2 简介
AI学习助手是一款集AI问答、阅读、笔记于一体的学习辅助软件，目前已更新至1.01版本。有了AI帮忙，加上一个简单好用的书籍、笔记系统，对于学习晦涩复杂的知识体系，比如哲学、政治经济学、精神分析、计算机原理、包括AI技术本身会更直白、高效、成体系。
...
![[editing toolbar.png]]
关于Editing Toolbar，请参考：[[#5.2.1 Editing Toolbar 格式工具栏]]。
...
><font color="#92d050">第三方插件</font>页面：![[第三方插件打开状态.png]]

下面是一些第三方插件功能简单介绍。
### 5.2.1 Editing Toolbar 格式工具栏

<font color="#92d050">Editing Toolbar</font>可以让用户像编辑word文档那样修改文字的格式。
![[editing toolbar.png]]

### 5.2.2 Floating Toc 悬浮目录
...
![[copilot_model.png]]
AI学习助手的正常使用需要配置两种模型，<font color="#92d050">对话模型（Chat Model)</font>和<font color="#92d050">嵌入模型（Embedding Model）</font>。上述两种模型的具体功能，请参考：[[#6.2 AI基本问答流程]]。
...
若要使用本地模型，推荐使用LM Studio下载和管理本地模型的使用和加载，详细请参考：[[#7.1 LM Studio]]
### 6.1.1 添加Chat Model
首先添加对话模型。在Model标签页Chat Model下面，点击Add Custom Model按钮。
![[添加chat model.png]]
弹出的模型配置窗口如下：
![[Add Custom Chat Model.png|300]]

配置以硅基流动API为例，请参考：[[#7.2 硅基流动]]
...
### 6.1.2 添加Embedding Model
除了Chat Model，还需要添加Embedding Model嵌入模型。

>[!note]
嵌入模型的主要功能是搜索仓库里面的文本，把同跟提问相关文本提交给对话模型。深入了解嵌入模型，请参考： [[#6.2 AI基本问答流程]]
如果注重隐私，可以加载本地嵌入模型，如需使用LM Studio软件，请参考： [[#7.1 LM Studio]]]
...
>![[chat_or_VaultQA.png|400]]
>
>用了VaultQA模式，AI会搜索你的仓库，在回答里面包含你笔记的内容，甚至使用你的图片:
>![[VaultQA_modelans.png|500]]
>
>可以选择不同的对话模型：
>![[选择对话模型.png|400]]
>
...
>......
>...最后点击![[reindex_current_note.png|15]]重建索引，让新卡片融入知识网络。需要具体案例参考[[哲学复习卡片范例]]第7节。（眼尾微挑）这种信息密度——跟得上么？
...
>本地下载使用AI模型，请参考[[#7.1 LM Studio]]部分。


### 6.2.1 deepseek R1工作流程
...
### 7.2.1 如何在Obsidian中接入硅基流动？
### 7.2.2 步骤1：注册并获取API密钥
...
### 7.2.3 步骤2：配置Obsidian Copilot插件

需要安装Copilot插件，打开Copilot插件，找到Model下的"add Custom Model"![[copilot添加模型.png]]
